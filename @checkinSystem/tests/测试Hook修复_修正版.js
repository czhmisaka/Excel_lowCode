/**
 * 测试Hook修复脚本 - 修正版
 * 使用正确的接口路径验证修复后的Hook功能
 */

const axios = require('axios');

// 配置
const CONFIG = {
  API_BASE: 'http://118.196.16.32:13000/api',
  FORM_ID: 'labor_sign_in',
  TABLE_HASH: '009297a8bd420455315d2b6529eb6f8d',
  TIMEOUT: 10000
};

/**
 * 测试表单定义获取
 */
async function testFormDefinition() {
  console.log('🔍 测试表单定义获取...');
  
  try {
    const response = await axios.get(
      `${CONFIG.API_BASE}/public/form/forms/${CONFIG.FORM_ID}`,
      {
        timeout: CONFIG.TIMEOUT
      }
    );
    
    if (response.data.success) {
      console.log('✅ 表单定义获取成功');
      console.log('表单名称:', response.data.data.name);
      console.log('表单描述:', response.data.data.description);
      console.log('表映射:', response.data.data.tableMapping);
      return true;
    } else {
      console.log('❌ 表单定义获取失败:', response.data.message);
      return false;
    }
  } catch (error) {
    console.error('❌ 表单定义获取失败:', error.response?.data || error.message);
    return false;
  }
}

/**
 * 测试表单提交 - 自动时间记录
 */
async function testAutoTimeHook() {
  console.log('🧪 测试自动时间记录Hook...');
  
  const testData = {
    data: {
      name: '测试用户',
      phone: '13906600231',
      company: 'huibo',
      // 不提供签到时间，让Hook自动填充
      sign_in_time: '',
      sign_out_time: '2025-11-17T10:00:00.000Z',
      work_hours: 0
    }
  };
  
  try {
    console.log('提交测试数据（不提供签到时间）...');
    const response = await axios.post(
      `${CONFIG.API_BASE}/public/form/forms/${CONFIG.FORM_ID}/submit`,
      testData,
      {
        timeout: CONFIG.TIMEOUT,
        headers: {
          'Content-Type': 'application/json'
        }
      }
    );
    
    if (response.data.success) {
      console.log('✅ 表单提交成功');
      console.log('响应数据:', JSON.stringify(response.data.data, null, 2));
      
      // 检查签到时间是否被自动填充
      if (response.data.data.sign_in_time && response.data.data.sign_in_time !== '') {
        console.log('✅ 自动时间记录Hook工作正常');
        console.log(`   签到时间: ${response.data.data.sign_in_time}`);
      } else {
        console.log('❌ 自动时间记录Hook未工作');
      }
      
      return true;
    } else {
      console.log('❌ 表单提交失败:', response.data.message);
      return false;
    }
  } catch (error) {
    console.error('❌ 测试失败:', error.response?.data || error.message);
    return false;
  }
}

/**
 * 测试工作时间计算Hook
 */
async function testWorkHoursHook() {
  console.log('🧪 测试工作时间计算Hook...');
  
  const testData = {
    data: {
      name: '测试用户2',
      phone: '13906600232',
      company: 'hengxin',
      sign_in_time: '2025-11-17T08:00:00.000Z',
      sign_out_time: '2025-11-17T12:00:00.000Z',
      work_hours: 0
    }
  };
  
  try {
    console.log('提交测试数据（提供签到和签退时间）...');
    const response = await axios.post(
      `${CONFIG.API_BASE}/public/form/forms/${CONFIG.FORM_ID}/submit`,
      testData,
      {
        timeout: CONFIG.TIMEOUT,
        headers: {
          'Content-Type': 'application/json'
        }
      }
    );
    
    if (response.data.success) {
      console.log('✅ 表单提交成功');
      console.log('响应数据:', JSON.stringify(response.data.data, null, 2));
      
      // 检查工作时间是否被计算
      if (response.data.data.work_hours && response.data.data.work_hours > 0) {
        console.log('✅ 工作时间计算Hook工作正常');
        console.log(`   工作时间: ${response.data.data.work_hours} 小时`);
      } else {
        console.log('❌ 工作时间计算Hook未工作');
      }
      
      return true;
    } else {
      console.log('❌ 表单提交失败:', response.data.message);
      return false;
    }
  } catch (error) {
    console.error('❌ 测试失败:', error.response?.data || error.message);
    return false;
  }
}

/**
 * 测试数据验证Hook
 */
async function testValidationHook() {
  console.log('🧪 测试数据验证Hook...');
  
  const testData = {
    data: {
      name: '', // 空姓名，应该触发验证错误
      phone: '12345678901', // 无效手机号格式
      company: '',
      sign_in_time: '2025-11-17T08:00:00.000Z',
      sign_out_time: '2025-11-17T12:00:00.000Z',
      work_hours: 0
    }
  };
  
  try {
    console.log('提交无效测试数据...');
    const response = await axios.post(
      `${CONFIG.API_BASE}/public/form/forms/${CONFIG.FORM_ID}/submit`,
      testData,
      {
        timeout: CONFIG.TIMEOUT,
        headers: {
          'Content-Type': 'application/json'
        }
      }
    );
    
    // 如果提交成功，说明验证Hook未工作
    if (response.data.success) {
      console.log('❌ 数据验证Hook未工作 - 无效数据被接受了');
      return false;
    } else {
      console.log('✅ 数据验证Hook工作正常');
      console.log(`   验证错误: ${response.data.message}`);
      return true;
    }
  } catch (error) {
    if (error.response?.status === 400) {
      console.log('✅ 数据验证Hook工作正常');
      console.log(`   验证错误: ${error.response.data.message}`);
      return true;
    } else {
      console.error('❌ 测试失败:', error.response?.data || error.message);
      return false;
    }
  }
}

/**
 * 测试模板字符串替换
 */
async function testTemplateReplacement() {
  console.log('🧪 测试模板字符串替换...');
  
  const testData = {
    data: {
      name: '模板测试用户',
      phone: '13906600233',
      company: 'temporary',
      // 使用模板字符串
      sign_in_time: '{{current_time}}',
      sign_out_time: '2025-11-17T18:00:00.000Z',
      work_hours: 0
    }
  };
  
  try {
    console.log('提交包含模板字符串的测试数据...');
    const response = await axios.post(
      `${CONFIG.API_BASE}/public/form/forms/${CONFIG.FORM_ID}/submit`,
      testData,
      {
        timeout: CONFIG.TIMEOUT,
        headers: {
          'Content-Type': 'application/json'
        }
      }
    );
    
    if (response.data.success) {
      console.log('✅ 表单提交成功');
      console.log('响应数据:', JSON.stringify(response.data.data, null, 2));
      
      // 检查模板字符串是否被替换
      if (response.data.data.sign_in_time && 
          response.data.data.sign_in_time !== '{{current_time}}' &&
          response.data.data.sign_in_time.includes('2025')) {
        console.log('✅ 模板字符串替换Hook工作正常');
        console.log(`   替换后的签到时间: ${response.data.data.sign_in_time}`);
      } else {
        console.log('❌ 模板字符串替换Hook未工作');
      }
      
      return true;
    } else {
      console.log('❌ 表单提交失败:', response.data.message);
      return false;
    }
  } catch (error) {
    console.error('❌ 测试失败:', error.response?.data || error.message);
    return false;
  }
}

/**
 * 主函数
 */
async function main() {
  console.log('🚀 开始测试Hook修复效果...');
  console.log('目标服务器:', CONFIG.API_BASE);
  console.log('表单ID:', CONFIG.FORM_ID);
  console.log('表哈希:', CONFIG.TABLE_HASH);
  console.log('='.repeat(50));
  
  try {
    // 1. 测试表单定义获取
    const formDefinitionTest = await testFormDefinition();
    
    console.log('');
    
    // 2. 测试自动时间记录Hook
    const autoTimeTest = await testAutoTimeHook();
    
    console.log('');
    
    // 3. 测试工作时间计算Hook
    const workHoursTest = await testWorkHoursHook();
    
    console.log('');
    
    // 4. 测试数据验证Hook
    const validationTest = await testValidationHook();
    
    console.log('');
    
    // 5. 测试模板字符串替换
    const templateTest = await testTemplateReplacement();
    
    console.log('='.repeat(50));
    console.log('📋 测试总结:');
    console.log(`- 表单定义获取: ${formDefinitionTest ? '✅ 正常' : '❌ 异常'}`);
    console.log(`- 自动时间记录Hook: ${autoTimeTest ? '✅ 正常' : '❌ 异常'}`);
    console.log(`- 工作时间计算Hook: ${workHoursTest ? '✅ 正常' : '❌ 异常'}`);
    console.log(`- 数据验证Hook: ${validationTest ? '✅ 正常' : '❌ 异常'}`);
    console.log(`- 模板字符串替换: ${templateTest ? '✅ 正常' : '❌ 异常'}`);
    
    const allTestsPassed = formDefinitionTest && autoTimeTest && workHoursTest && validationTest && templateTest;
    
    if (allTestsPassed) {
      console.log('🎉 所有Hook功能测试通过！');
      console.log('');
      console.log('💡 修复效果:');
      console.log('1. 时间自动记录功能已修复');
      console.log('2. 工作时间计算功能已修复');
      console.log('3. 数据验证功能已修复');
      console.log('4. {{current_time}} 模板问题已解决');
      console.log('5. Hook执行流程正常');
    } else {
      console.log('⚠️ 部分Hook功能仍需调试');
    }
    
  } catch (error) {
    console.error('❌ 测试过程中发生错误:', error.message);
    process.exit(1);
  }
}

// 执行测试
if (require.main === module) {
  main()
    .then(() => {
      process.exit(0);
    })
    .catch(error => {
      console.error('测试过程中发生错误:', error);
      process.exit(1);
    });
}

module.exports = {
  testFormDefinition,
  testAutoTimeHook,
  testWorkHoursHook,
  testValidationHook,
  testTemplateReplacement
};
