...
const COMPANIES = [
  {
    ...
    tableMapping: 'huibo_labor_sign_records'
  },
  {
    id: 'hengxin', 
    name: '恒信劳务公司',
    ...
    tableMapping: 'hengxin_labor_sign_records'
  },
  {
    id: 'temporary',
    name: '临时工',
    ...
    tableMapping: 'temporary_labor_sign_records'
  }
];
 ...
 * 创建签到表单定义
 */
async function createCheckinForm(company) {
  ...
  const formData = {
    ...
    tableMapping: company.tableMapping,
    definition: {
      fields: [
        {
          name: 'name',
 ...
 * 创建签退表单定义
 */
async function createCheckoutForm(company) {
 ...
 * 主函数 - 创建简化的二维码签到系统
 */
async function createSimpleQRCodeSystem() {
  try {
    console.log('🚀 开始创建简化的二维码签到系统...');
    ...
    for (const company of COMPANIES) {
      ...
      // 1. 创建签到表单
      await createCheckinForm(company);
      
      // 2. 创建签退表单
      await createCheckoutForm(company);
    ...
    for (const company of COMPANIES) {
      ...
      console.log(`  - 数据表: ${company.tableMapping}`);
    }
