...
async function checkServerConnection() {
  ...
  try {
    ...
    console.log('请检查:');
    console.log('1. 服务器地址是否正确: http://localhost:4000');
...
async function validateSystem() {
  ...
  try {
    ...
    console.log(`${CONFIG.API_BASE.replace('/api', '')}/api/public/form/forms/labor_sign_in`);
...
async function deploySignInSystem() {
  ...
  if (systemValidated) {
    ...
    console.log('');
    console.log('🔗 访问地址:');
    console.log(`公开表单: ${CONFIG.API_BASE.replace('/api', '')}/api/public/form/forms/labor_sign_in`);
    console.log('');
    console.log('💡 使用说明:');
    console.log('1. 用户可以通过公开表单地址进行签到签退');
    console.log('2. 系统会自动记录时间和计算工作时间');
    console.log('3. 支持重复签到验证');
  } else {
